...
# Example Usage (for testing this dialog itself)
if __name__ == '__main__':
    ...
    print("Matplotlib Current rcParams Initial Values (Dialog will display based on these):")
    for key in [
        "font.family", "font.size", "text.usetex", 
        "mathtext.fontset", "mathtext.rm", "mathtext.it", "mathtext.bf", "mathtext.bfit", # MathText
        "text.fontfamily", "text.fontsize", "text.fontweight", "text.fontstyle", # ax.text
        "lines.linewidth", "lines.linestyle", "lines.marker", "lines.markersize",
        "axes.labelsize", "xtick.labelsize", "ytick.labelsize", "axes.grid",
        "legend.fontsize", "legend.frameon", "legend.loc",
        "figure.figsize", "savefig.dpi", "savefig.format"
    ]:
        print(f"{key}: {plt.rcParams.get(key)}")

    dialog = MatplotlibSettingsDialog()

    def handle_applied_settings(settings):
        print("\n=== Matplotlib Settings Obtained and Applied from Dialog ===")
        print("Settings obtained from dialog:", settings)
        try:
            for key, value in settings.items():
                if key == "font.family" and isinstance(value, str):
                    plt.rcParams[key] = [value]
                elif key == "figure.figsize" and isinstance(value, list):
                    plt.rcParams[key] = value
                elif key == "lines.marker" and value == 'None':
                    plt.rcParams[key] = None
                # --- MathText and ax.text related settings logic ---
                elif key.startswith("mathtext.") or key.startswith("text."):
                    if settings.get("text.usetex"):
                        # If usetex is True, remove all mathtext.* and text.* (except text.usetex itself)
                        if key in plt.rcParams and key != "text.usetex": 
                            del plt.rcParams[key]
                    elif value: # If usetex is False and value is not empty, then set
                        plt.rcParams[key] = value
                    else: # If usetex is False and value is empty, then delete or reset
                        if key in plt.rcParams:
                            if key.startswith("mathtext."):
                                del plt.rcParams[key] 
            ...
            # Force refresh Matplotlib font cache
            try:
                fm._fm_config_cache.clear()
                fm.findfont.cache_clear()
                fm.get_fontconfig_fonts()
                print("Matplotlib font cache refreshed.")
            except Exception as e:
                print(f"Error refreshing Matplotlib font cache: {e}")

            print("Matplotlib rcParams updated successfully.")
        except Exception as e:
            print(f"Error applying Matplotlib settings: {e}")

    dialog.settings_applied.connect(handle_applied_settings)

    if dialog.exec() == QDialog.Accepted:
        print("\nDialog accepted and closed.")
    else:
