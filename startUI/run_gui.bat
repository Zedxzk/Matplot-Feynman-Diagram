...
for /f "tokens=2 delims==" %%a in ('set CONDA_PROMPT_MODIFIER 2^>nul') do (
    set "CONDA_PROMPT_MODIFIER=%%a"
)
if defined CONDA_PROMPT_MODIFIER (
    echo 检测到已激活的 Conda 环境：!CONDA_PROMPT_MODIFIER!
) else (
    echo 未检测到 Conda 环境，使用系统默认 Python。
)
